...
export function RegisterSuccess({
  ...
  className?: string
  back: () => void
  successButton: { text: string, onClick: () => void }
}) {
  ...
  return (
    <Modal
      ...
      maskClosable={false}
      className={cn(
        'login-dialog [&>div>.ant-modal-content]:!bg-#0D1117',
        className
      )}
      centered
      title={(
    ...
    >
      <div className="fccc p-4">
        ...
        <div className="mt-4.5 text-10 text-white font-600 leading-15">
          {t('register.status.success')}
        </div>
        <div className="mt-2 text-lg text-#D1D5DB">
          {t('register.status.accountActivated')}
        </div>
        <div className="mt-6 fcc gap-2 b b-#00FF85 rounded-9999px b-solid px-4 py-2">
          ...
          <div className="text-#00FF85">
            {t('register.status.enterpriseVerified')}
          </div>
        </div>
        <div className="mt-8 fyc gap-4">
          <Button
            onClick={back}
            className="h-13.5 min-w-45 b-#374151 bg-#000000 text-4 font-600"
          >
            {t('register.status.backToHome')}
          </Button>
          <Button
            className="h-13.5 min-w-45 b-#00E5FF80 bg-#00E5FF text-4 text-black font-600"
  ...
  return (
    ...
    >
      <div className="fccc p-4">
        ...
        <div className="mt-4.5 text-10 text-white font-600 leading-15">
          {t('register.status.failed')}
        </div>
        <div className="mt-2 text-base text-#8B949E">{errorMessage}</div>
        <div className="mt-8 w-full fyc gap-4">
          <Button
            onClick={() => setVisible(false)}
            className="h-13.5 min-w-45 w-full b-#374151 bg-#000000 text-4 font-600"
          >
            {t('register.status.retry')}
          </Button>
        </div>
