...
import { Alert, Button, Collapse, Image, Input, InputNumber, Select, Spin } from 'antd'
...
function RouteComponent() {
  ...
  const { id: assetId } = useParams({ from: '/_app/assete/info/$id' }) as { id: string }
  ...
  const getAuditorName = (data: AuditLog | undefined) => {
    ...
    switch (lastAuditLog.auditor_role) {
      case ASSET_AUDITOR_ROLE.ASSET_OWNER:
        return {
          name: t('common.identity.assetParty', { name: lastAuditLog?.auditor_name }),
          position: t('common.identity.assetPartyContent')
        }
      case ASSET_AUDITOR_ROLE.LAWYER:
        return {
          name: t('common.identity.lawyer', { name: lastAuditLog?.auditor_name }),
          position: t('common.identity.lawyerContent')
        }
      case ASSET_AUDITOR_ROLE.ASSESSOR:
        return {
          name: t('common.identity.evaluator', { name: lastAuditLog?.auditor_name }),
          position: t('common.identity.evaluatorContent')
        }
      case ASSET_AUDITOR_ROLE.PLATFORM_ADMIN:
        return {
          name: t('common.identity.platformAdmin', { name: lastAuditLog?.auditor_name }),
  ...
  const [correctionAssetData, setCorrectionAssetData] = useState<SubmitAssetInfo>({} as any)
  // 监听资产信息,更新资产信息
  useEffect(() => {
    if (assetInfo) {
      setCorrectionAssetData({
        ...
        basic_info: {
          ...
          annual_return_max: Number(assetInfo?.properties?.annual_return_max || ''),
          annual_return_min: Number(assetInfo?.properties?.annual_return_min || ''),
          area: Number(assetInfo?.properties?.area || ''),
          city: Number(assetInfo?.properties?.city || ''),
          expected_annual_return: Number(assetInfo?.properties?.expected_annual_return || ''),
        ...
        files: assetInfo?.files.map((file) => {
          ...
          return {
            fields,
            file_urls: file.file_urls.status !== REVIEW_STATUS.REJECTED ? file.file_urls.fileUrls : [],
  ...
  const asseteBasicInformation = useMemo(() => {
    const assetType = assetHouseTypeData?.find(item => item.value === Number(correctionAssetData.basic_info?.property_type))
    return [
      { title: 'assete.info.houseName', value: correctionAssetData.basic_info?.name, name: 'name', disable: false },
      { title: 'assete.info.address', value: correctionAssetData.basic_info?.address, name: 'address', disable: false },
      { title: 'assete.info.houseType', value: assetType?.label, name: 'property_type', disable: false },
      { title: 'assete.info.assetCode', value: assetInfo?.submission?.code, name: 'code', disable: true },
      { title: 'assete.info.appraisalPrice', value: assetInfo?.properties?.price, name: 'evaluationPrice', disable: true },
      { title: 'assete.info.area', value: `${correctionAssetData.basic_info?.area}`, name: 'area', disable: false },
      { title: 'assete.info.ownership', value: assetInfo?.submission?.assets_user_name || '--', name: 'owner', disable: true },
      {
        title: 'assete.info.submitDate',
        value: assetInfo?.properties?.created_date
          ? dayjs(assetInfo?.properties?.created_date * 1000).format('YYYY-MM-DD HH:mm:ss')
          : '',
        name: 'submissionDate',
        disable: true
      }
  ...
  function rejectedFiles(template_file_id: number) {
    return correctionAssetData?.files?.find(file => file.template_file_id === template_file_id)
  }
  ...
  async function uploadIdCardFile(data: File, template_file_id: number) {
    ...
    await uploadIdCardFileMutate({ file: data }).then((res) => {
      if (res.code === 1) {
        const urls = [
          ...correctionAssetData.files.find(file => file.template_file_id === template_file_id)?.file_urls || [],
      ...
      setUploadFileLoading(prev => prev.filter(item => item !== template_file_id))
    })
  }
  ...
  const { mutateAsync: saveAssetInfo, isPending: saveAssetInfoIsPending } = useMutation({
    ...
    <div className="px-22 py-8 max-md:px-4">
      ...
      {assetInfo?.submission?.status === ASSET_SUBMISSION_STATUS.REJECTED && (
        <Alert
          ...
          message={<div className="text-base text-white font-600">{assetAuditLog?.reject_reason}</div>}
          description={<div className="text-sm text-#D1D5DB">{assetAuditLog?.remark}</div>}
          type="error"
          showIcon
          closable
        />
      ...
      <div className="mt-6 b b-#334155 rounded-3 b-solid bg-#2E384880 p-6 backdrop-blur-4">
        <div className="flex flex-col gap-4 b-b-1 b-#374151 b-solid pb-6">
          <div className="fyc justify-between gap-4">
            <div className="fyc gap-4">
              <div className="text-3xl font-bold">{assetInfo?.properties?.name}</div>
          ...
          <div className="grid cols-3 mt-4 gap-6">
            <div className="fyc gap-4">
              ...
              <div>
                ...
                <div className="text-base font-bold">{assetInfo?.submission?.code}</div>
              </div>
            </div>
            <div className="fyc gap-4">
              <div className="h-13 w-11 fcc rounded-2 bg-#0A246480">
            ...
            <div className="fyc gap-4">
              ...
              <div>
                ...
                <div className="text-base font-bold">
                  {
                    (getAuditorName(assetInfo?.audit_logs?.[assetInfo?.audit_logs?.length - 1])?.name) || '--'
                  }
                </div>
              </div>
            </div>
        ...
        <div className="pb-16 pt-6">
          <div className="text-xl font-600">{t('assete.info.assetBasicInfo')}</div>
                          ...
                          ['area'].includes(item.name)
                            ...
                            : (
                                item.name === 'property_type'
                                  ...
                                  : <Input onChange={e => changeCorrectionAssetData(e.target.value, item.name)} defaultValue={item.value || ''} className="h-12.5 b-#374151 bg-#1E2328"></Input>
                              )
                        )
                      : (
                          <div>
                            {item.name === 'evaluationPrice'
                              ? `$${formatNumberNoRound(Number(item.value || ''), 6, 2)}`
                              : (
        ...
        <div className="flex-[68]">
            ...
            <Panel
              ...
              header={(
                <div className="fyc gap-4">
                  ...
                  <div className="rounded-9999px bg-#334155 px-3 py-1 text-xs text-#00FF85">{t('common.item', { num: classifyFiles.approved.length })}</div>
                </div>
              )}
              key="1"
            >
                ...
                classifyFiles.approved.length > 0
                  ? classifyFiles.approved.map(file => (
                      <FileContent
                        ...
                        fileType={getFileExtension(file?.file_urls?.fileUrls[0] || '')}
                        showImage={() => {
                          setShowImage({ visible: true, url: file.file_urls.fileUrls })
                        }}
                      />
                    ))

                  : <div className="text-center text-base text-#D1D5DB">{t('common.noContent')}</div>
              }
            </Panel>
          </Collapse>
            ...
            <Panel
              ...
              header={(
                <div className="fyc gap-4">
                  ...
                  <div className="rounded-9999px bg-#FF4B5033 px-3 py-1 text-xs text-#FF4D4F">{t('common.item', { num: classifyFiles.rejected.length })}</div>
                </div>
              )}
            >
                    ...
                    ? classifyFiles.rejected.map((item) => {
                        return (
                          ...
                          <div className="fyc gap-1 text-sm text-#00E5FF clickable">
                            ...
                            {
                              rejectedFiles(item?.template_file_id || 0)?.file_urls.map((file, _index) => (
                                <FileContent
                                  key={file}
                                  fileName={`${item?.file_info?.display_name}-${_index + 1}`}
                            ...
                            <div className="mt-4 b b-#FF4C4F4D rounded-2 b-solid bg-#0A24634D p-5">
                              <div className="fyc justify-between">
                                <div className="text-lg">{item?.file_info?.display_name}</div>
                            ...
                            {}
                              ...
                              <UploadMultifileCard
                                ...
                                width="100%"
                                height="auto"
                                loading={uploadFileLoading.includes(item.template_file_id)}
                                beforeUpload={(_file) => {
          ...
          >
            <Panel
              ...
              header={(
                <div className="fyc gap-4">
                  <div className="h-13 w-11 fcc rounded-2 bg-#FFD70033"><div className="i-icon-park-solid:audit bg-#FFD700 text-5"></div></div>
                  <div className="text-xl font-600">{t('assete.info.pendingAuditFiles')}</div>
                  <div className="rounded-9999px bg-#FFD70033 px-3 py-1 text-xs text-#FFD700">{t('common.item', { num: classifyFiles.pending.length })}</div>
                </div>
              )}
              key="1"
            >
        ...
        <div className="flex-[32]">
          ...
          <div className="rounded-3 bg-#2E384880 p-6 backdrop-blur-4">
            <div className="fyc gap-1.5">
              ...
              <div className="text-xl font-600">{t('assete.info.auditRemark')}</div>
            </div>
            {assetInfo?.audit_logs.map((log, _index) => (
              <div key={log.id} className="mt-4 b b-#00E6FF33 rounded-2 b-solid bg-#0A24634D p-5">
                ...
                <div className="mt-4 fyc justify-between gap-2 b-t-1 b-t-#38425280 b-t-solid pt-5">
                  <div className="truncate text-xs text-#6B7280">{log?.audit_time && dayjs(log?.audit_time * 1000).format('YYYY-MM-DD HH:mm:ss')}</div>
          ...
          <div className="mt-8 rounded-3 bg-#2E384880 p-6 backdrop-blur-4">
            <div className="fyc gap-1.5">
              ...
              <div className="text-xl font-600">{t('assete.info.auditProgress')}</div>
            </div>
            <div className="mt-4">
              {
                assetInfo?.audit_logs.map((item) => {
                  ...
                  return (
                    <div className="flex gap-4" key={item.id}>
                      ...
                      <div>
                        <div className="text-base font-500">{item.status <= 2 ? t(`assete.info.progress.${item.status}.title`) : item.reject_reason}</div>
                        <div className="text-sm text-#9CA3AF">{dayjs().format('YYYY-MM-DD HH:mm:ss')}</div>
                        <div className="mt-1 text-base text-#D1D5DB">
                          {item.status <= 2
                            ? t(`assete.info.progress.${item.status}.description`, { name: item?.auditor_name })
                            : item.remark}
                        </div>
                      </div>
                    </div>
      ...
      <div className="mt-40 flex justify-end">
        {assetInfo?.submission?.status === ASSET_SUBMISSION_STATUS.REJECTED
          ? (
              <Button loading={saveAssetInfoIsPending} onClick={updateAsset} className="h-10.5 b-#00E5FF bg-#00E5FF px-11 text-base text-#0D1117 font-600">
