      ...
      <div className="mt-4 py-6 text-white">
          ...
          <div className="rounded-999px bg-#CD647833 px-2 py-1 text-xs text-#CF6679 font-500">
            {t(
              `common.assetPropertyStatus.${
                data?.property_status < 0 || data?.property_status > 8
                  ? 'other'
        ...
        <div className="grid cols-2 mt-6 gap-6 text-sm [&>div>div:first-child]:text-#9CA3AF">
          ...
          <div>
            ...
            <div>{dayjs(data?.next_rent_date || '').format('YYYY-MM-DD')}</div>
          </div>
          <div>
            <div>{t('assete.defaultDetails.overdueDays')}</div>
            <div className="font-600">
              {data?.breaking_contract_number || 1}
          ...
          <div>
            ...
            <div>
              ...
              {formatNumberNoRound(
                ...
                || 0
                * Math.max(
                  Number.parseInt(`${data.breaking_contract_number / 30}`),
                  1
                ),
                6,
                2
              ...
              <div key={item} className="w-full fyc gap-4">
                <div className="fccc gap-1">
                  ...
                  <div
                    className={cn(
                      'h-13 w-0.5',
                      item < 2 ? 'bg-#374151' : 'bg-transparent'
                    )}
                  >
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-1 text-xs text-#6B7280">
                    {dayjs().add(item, 'day').format('YYYY-MM-DD HH:mm')}
                  </div>
                </div>
              </div>
            ))}
