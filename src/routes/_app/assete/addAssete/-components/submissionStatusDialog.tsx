...
export function SubmissionStatusStatus({
  ...
  return (
    type
      ? <SuccessDialog setVisible={setVisible} visible={visible} asseteId={asseteInfo.code} createTime={asseteInfo.createTime} />
      : <ErrorDialog setVisible={setVisible} visible={visible} asseteId={asseteInfo.code} createTime={asseteInfo.createTime} errorMessage={errorMessage || ''} />
  )
}

export function SuccessDialog({ visible, setVisible, asseteId, createTime }:
{ visible: boolean, setVisible: (visible: boolean) => void, asseteId: string, createTime: string }) {
      ...
      <div className="w-full fccc p-4">
        ...
        <div className="mt-8 w-full b-1 b-#00E6FF33 rounded-2 b-solid bg-#00000080 p-6 text-base text-#9CA3AF">
          <div className="fyc justify-between gap-2">
            ...
            <div className="flex-1 truncate text-right text-#00E5FF font-bold">{asseteId}</div>
          </div>
...
export function ErrorDialog({ visible, setVisible, asseteId, createTime, errorMessage }:
{ visible: boolean, setVisible: (visible: boolean) => void, asseteId: string, createTime: string, errorMessage: string }) {
  const navigate = useNavigate()
  const { t } = useTranslation()
