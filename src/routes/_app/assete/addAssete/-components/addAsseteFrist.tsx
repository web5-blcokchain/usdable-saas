...
export function AddAsseteFrist({
  form,
  onFinish
}: {
  ...
  const { isFetching: locationDataLoading } = useQuery({
    ...
    queryFn: async () => {
      const data = await getLocation({
        ...
        parent_id:
          selectPid.selectedLocation === 1 ? selectPid.country : undefined
      })
      // 处理数据,
      const newData
        = data?.data?.map(item => ({
          ...
          label: i18n.language === 'zh' ? item.name : item.name_en || item.name,
          code: item.code
        })) || ([] as LocationData[])
    ...
    <div className="bg-#000000 px-22 pb-15 pt-8 max-md:px-4">
      <div onClick={back} className="w-fit fcc gap-1 clickable">
        ...
        <div className="text-2xl font-600">
          {t('assete.addAsset.uploadNewAsset')}
        </div>
      </div>
      <Form layout="vertical" form={form} onFinish={onFinish}>
            ...
            <div className="grid cols-4 mt-6 gap-6 max-md:cols-1 max-md:gap-4">
              {assetTypeList.map((item, index) => (
                <div
                  key={index}
                  className={cn(
                    ' fccc b-1 b-solid py-6 px-4 rounded-2 relative',
                    assetType === item.id ? 'b-#00E5FF' : ' b-#374151',
                    item.status === 1 ? 'clickable' : '  cursor-not-allowed'
                  )}
                  onClick={() => {
                    if (item.status === 1)
        ...
        <div className="mt-10 rounded-3 bg-#161B22 px-6 py-7.5 pb-10">
          ...
          <div className="grid cols-2 mt-6 gap-6">
            ...
            <Form.Item
              name="name"
              label={t('assete.addAsset.assetName')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.assetNamePlaceholder')
                }
              ]}
            >
            ...
            <Form.Item
              name="property_type"
              label={t('assete.addAsset.propertyTypeLabel')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.propertyTypePlaceholder')
                }
              ]}
            >
              <Select
            ...
            <Form.Item
              name="area"
              label={t('assete.addAsset.buildingArea')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.buildingAreaPlaceholder')
                }
              ]}
            >
              <InputNumber
                min={1}
                controls={false}
                placeholder={t('assete.addAsset.buildingAreaPlaceholder')}
              />
            </Form.Item>
            {/* 资产地址 */}
            <Form.Item
              name="address"
              label={t('assete.addAsset.assetAddress')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.assetAddressPlaceholder')
                }
              ]}
            >
              <Input
                placeholder={t('assete.addAsset.assetAddressPlaceholder')}
              />
            </Form.Item>
            {/* 建筑年限 */}
            <Form.Item
              name="house_life"
              label={t('assete.addAsset.buildingYear')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.buildingYearPlaceholder')
                }
              ]}
            >
              <InputNumber
                ...
                max={200}
                controls={false}
                placeholder={t('assete.addAsset.buildingYearPlaceholder')}
              />
            </Form.Item>
            {/* 资产估值 */}
            ...
            <Form.Item
              name="bedrooms"
              label={t('assete.addAsset.bedroomCount')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.bedroomCountPlaceholder')
                }
              ]}
            >
              <InputNumber
                ...
                max={100}
                controls={false}
                placeholder={t('assete.addAsset.bedroomCountPlaceholder')}
              />
            </Form.Item>
            {/* 缴租日期 */}
            <Form.Item
              name="rent_day"
              label={t('assete.addAsset.rentDate')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.rentDatePlaceholder')
                }
              ]}
            >
              <Select
            ...
            <Form.Item
              name="chain_id"
              label={t('assete.addAsset.chainId')}
              required
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.chainIdPlaceholder')
                }
              ]}
            >
              <InputNumber
                ...
                min={1}
                controls={false}
                placeholder={t('assete.addAsset.chainIdPlaceholder')}
              />
            </Form.Item>
            {/* 月租金 */}
            <Form.Item
              name="monthly_rent"
              label={t('assete.addAsset.monthlyRent')}
            >
              <InputNumber
                min={1}
                controls={false}
                placeholder={t('assete.addAsset.monthlyRentPlaceholder')}
              />
            </Form.Item>
            {/* 预计年回报率 */}
            <Form.Item
              name="expected_annual_return"
              label={t('assete.addAsset.expectedAnnualReturn')}
            >
              <InputNumber
                min={1}
                controls={false}
                placeholder={t(
                  'assete.addAsset.expectedAnnualReturnPlaceholder'
                )}
              />
            </Form.Item>
            {/* 预计年化收益下限 */}
            <Form.Item
              name="annual_return_min"
              label={t('assete.addAsset.expectedAnnualReturnLower')}
            >
              <InputNumber
                min={1}
                max={100}
                controls={false}
                placeholder={t(
                  'assete.addAsset.expectedAnnualReturnLowerPlaceholder'
                )}
              />
            </Form.Item>
            {/* 预计年化收益上限 */}
            <Form.Item
              name="annual_return_max"
              label={t('assete.addAsset.expectedAnnualReturnUpper')}
            >
              <InputNumber
                min={1}
                max={100}
                controls={false}
                placeholder={t(
                  'assete.addAsset.expectedAnnualReturnUpperPlaceholder'
                )}
              />
            </Form.Item>
            {/* 经度 */}
            <Form.Item name="longitude" label={t('assete.addAsset.longitude')}>
              <InputNumber
                ...
                max={180}
                controls={false}
                placeholder={t('assete.addAsset.longitudePlaceholder')}
              />
            </Form.Item>
            {/* 纬度 */}
            <Form.Item name="latitude" label={t('assete.addAsset.latitude')}>
              <InputNumber
                ...
                max={90}
                controls={false}
                placeholder={t('assete.addAsset.latitudePlaceholder')}
              />
            </Form.Item>
            {/* 邮编 */}
            <Form.Item
              name="postcode"
              required
              label={t('assete.addAsset.zipCode')}
              rules={[
                {
                  required: true,
                  message: t('assete.addAsset.zipCodePlaceholder')
                }
              ]}
            >
              <InputNumber
                min={1}
                controls={false}
                placeholder={t('assete.addAsset.zipCodePlaceholder')}
              />
            </Form.Item>
            {/* 国家编码 */}
            <Form.Item
              name="country_id"
              required
              label={t('assete.addAsset.countryCode')}
              rules={[
                {
                  required: true,
            ...
            <Form.Item
              name="city"
              required
              label={t('assete.addAsset.cityCode')}
              rules={[
                {
                  required: true,
          ...
          <Form.Item
            ...
            label={t('assete.addAsset.propertyDescription')}
            required
            rules={[
              {
                required: true,
                message: t('assete.addAsset.propertyDescriptionPlaceholder')
              }
            ]}
          >
            <Input.TextArea
              placeholder={t('assete.addAsset.propertyDescriptionPlaceholder')}
          ...
          <Form.Item
            name="location"
            label={t('assete.addAsset.locationDescription')}
          >
            <Input
              placeholder={t('assete.addAsset.locationDescriptionPlaceholder')}
            />
          </Form.Item>
        </div>
        <Form.Item>
