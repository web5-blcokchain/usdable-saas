...
export function LawOfficeRegister({ back }: { back: () => void }) {
  ...
  return <div className="mt-6 px-66 max-md:px-4 fccc pb-53">
    ...
    <div className='text-12 max-md:text-3xl font-600 leading-18 text-center'>{t('register.lawOffice.title')}</div>
    <div className='w-full rounded-lg mt-10 p-12 max-md:px-4 b-2 b-solid b-#31363c bg-#161B2199'>
      <div className="text-lg font-600 fyc gap-3">
      ...
      <Form form={form} layout="vertical" className="mt-6" onFinish={onFinish} onFinishFailed={onFinishFailed} >
        <div className='grid cols-2 max-md:cols-1 gap-x-6 max-md:gap-0'>
          <Form.Item required name="name" label={t('register.lawOffice.officeName')} rules={[{
            required: true,
            message: <div>{t('register.lawOffice.officeNamePlaceholder')}</div>
          }]}>
            <Input className="h-12.5 bg-#1E2328 b-#374151" placeholder={t('register.lawOffice.officeNamePlaceholder')} />
          </Form.Item>
        ...
        <Form.Item required name="licenseNumber" label={t('register.lawOffice.licenseNumber')} rules={[{ required: true, message: t('register.lawOffice.licenseNumberPlaceholder') }]}>
          <div className=' relative'>
            <Input className="h-12.5 bg-#1E2328 b-#374151" placeholder={t('register.lawOffice.licenseNumberPlaceholder')}></Input>
            <Button className=" absolute top-1/2 right-3 transform-translate-y--50% px-3 py-1 text-3 b-#00E6FF33 bg-#00E6FF33 text-#00E5FF">验证</Button>
          </div>
        </Form.Item>
        <div className='grid cols-2 max-md:cols-1 gap-x-6 max-md:gap-0'>
          <Form.Item required name="contact" label={t('register.lawOffice.contact')} rules={[{ required: true, message: t('register.lawOffice.contactPlaceholder') }]}>
            <Input className="h-12.5 bg-#1E2328 b-#374151" placeholder={t('register.lawOffice.contactPlaceholder')} />
          </Form.Item>
        </div>
        <Divider className='my-9 max-md:my-6' />
        <div className="text-lg font-600 fyc gap-3 mb-6">
          <img src={lawyerIcon} className='h-6' alt="" />
          <div>{t('register.lawOffice.leadLawyerInfo')}</div>
        </div>
        <div className='grid cols-2 max-md:grid-cols-1 gap-x-6 max-md:gap-0'>
          <Form.Item required name="lawyeName" label={t('register.lawOffice.lawyerName')} rules={[{ required: true, message: t('register.lawOffice.lawyerNamePlaceholder') }]}>
            <Input className="h-12.5 bg-#1E2328 b-#374151" placeholder={t('register.lawOffice.lawyerNamePlaceholder')} />
          </Form.Item>
          <Form.Item required name="lawyeLicenseNumber" label={t('register.lawOffice.licenseNo')} rules={[{ required: true, message: t('register.lawOffice.licenseNoPlaceholder') }]}>
            <Input className="h-12.5 bg-#1E2328 b-#374151" placeholder={t('register.lawOffice.licenseNoPlaceholder')} />
          </Form.Item>
        ...
        <Form.Item required name="certificates" rules={[{
          // 自定义验证
          validator: fileValidator,
        }]}>
          <div className='fccc [&>div]:w-full gap-6 '>
            <UploadMultifileCard
              className={cn(
              ...
              <div className="px-6.5 w-full">
                ...
                <div className="py-10 w-full bg-#1E242880 px-3 rounded-2">
                  <div className="fcc pb-2">
                    <img className='h-10' src={new URL('@/assets/images/register/cloud.png', import.meta.url).href} alt="" />
                  </div>
                  <div className="text-center text-sm text-#9CA3AF overflow-hidden w-full">{t('register.lawOffice.uploadOfficeLicense')}</div>
                  <div className="text-center text-xs text-#6B7280   overflow-hidden w-full">{t('register.lawOffice.fileSupport')}</div>
                </div>
              </div>
            </UploadMultifileCard>
            <UploadMultifileCard
              ...
              <div className="px-6.5 w-full">
                ...
                <div className="py-10 w-full bg-#1E242880 px-3 rounded-2">
                  ...
                  <div className="text-center text-sm text-#9CA3AF overflow-hidden w-full">{t('register.lawOffice.uploadLawyerLicense')}</div>
                  <div className="text-center text-xs text-#6B7280   overflow-hidden w-full">{t('register.lawOffice.fileSupport')}</div>
                </div>
              </div>
            </UploadMultifileCard>
            <UploadMultifileCard
              ...
              <div className="px-6.5 w-full">
                ...
                <div className="py-10 w-full bg-#1E242880 px-3 rounded-2">
                  ...
                  <div className="text-center text-sm text-#9CA3AF overflow-hidden w-full">{t('register.lawOffice.uploadBarProof')}</div>
                  <div className="text-center text-xs text-#6B7280   overflow-hidden w-full">{t('register.lawOffice.fileSupport')}</div>
                </div>
              </div>
            </UploadMultifileCard>
        ...
        <div className='mt-6 p-6 rounded-2 bg-#1E242880 '>
          <div className='text-base mb-3'>{t('register.lawOffice.complianceCommitment')}</div>
          <Form.Item valuePropName="checked" name="agreementFirst" className="mb-1" rules={[{ required: true, message: t('register.lawOffice.readServiceAgreement') }]}>
            <div className='fyc'>
              <Checkbox className="text-sm text-#D1D5DB">{t('register.lawOffice.agreeTerms')}<span className="text-#00E5FF">《{t('register.lawOffice.legalServiceAgreement')}》</span></Checkbox>
        ...
        <div className='mt-6 p-6 bg-#00FF891A b b-solid b-#00FF844D rounded-2 '>
          ...
          <div className='ml-6 mt-2 text-#8B949E text-sm'>
