...
export function OtherHeader() {
  ...
  const changeSelectNav = (index: number) => {
    ...
    if (index === 0) mainDom?.scrollTo({
      top: 0,
      behavior: 'smooth'
    })
    if (mainDom && moveDom && headerDom) {
      mainDom?.scrollTo({
        top: (moveDom?.getBoundingClientRect().top - headerDom?.getBoundingClientRect().height) + mainDom.scrollTop || 0,
        behavior: 'smooth'
      })
    }
  }
  ...
  return (
    <header className="sticky top-0 z-99 w-full fyc justify-between bg-black px-22 py-3 max-md:px-4 max-md:py-2" id="other-header">
      <div className="fyc gap-14">
        <img onClick={() => changeSelectNav(0)} className="h-14 max-md:h-12 cursor-pointer" src={logo} alt="" />
      ...
      <div className='fyc gap-4'>
        ...
        <Link to="/register" className='max-md:hidden'>
          <Button className="h-10 max-md:h-8 max-md:text-sm b-#00E5FF rounded-1.5 bg-#00E5FF px-4 text-base text-#0A0A0A font-400">{t('header.startUsing')}</Button>
        </Link>
      </div>
    </header>
  )
...
function MenuContent() {
  ...
  return (<div>
    <div onClick={() => setIsMenuOpen(!isMenuOpen)} className="i-mingcute:menu-line text-4 hidden max-md:block"></div>
    <AnimatePresence mode="wait">
      {isMenuOpen ? (
        ...
        >
          ...
          >
            <div className='text-xl fol items-end'>
              {
                navList.map((item, index) => (
                  <div onClick={() => changeSelectNav(index)} key={index} className='font-400 clickable mt-4 w-fit'>{item}</div>
                ))
              }
            </div>
          </div>
